...
User request
     │
     ├─ 1. Load (or cache) top 3 liked categories
     │
     ├─ 2. Filter out ineligible posts
     │
     ├─ 3. Score each post across 6 signals
     │
     ├─ 4. Keep only the best post per author
     │
     └─ 5. Return the top N results by score
```